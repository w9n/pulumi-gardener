...
First, clone this repo with the name of the desired provider in place of `gardener`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-gardener
```

Second, replace references to `gardener` with the name of your provider:
...
To use from Go, use `go get` to grab the latest version of the library
    ...
    $ go get github.com/pulumi/pulumi-gardener/sdk/go/...

## Configuration

The following configuration points are available for the `gardener` provider:

- `gardener:apiKey` (environment: `XYZ_API_KEY`) - the API key for `gardener`
- `gardener:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
